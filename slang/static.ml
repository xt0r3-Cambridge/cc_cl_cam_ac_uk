...
let match_types (t1, t2) = (t1 = t2 || t1 = TEany || t2 = TEany) 
...
let make_if loc (e1, t1) (e2, t2) (e3, t3) = 
  ...
  | TEbool | TEany -> 
    ...
    then (If(loc, e1, e2, e3), if t2 = TEany then t3 else t2) 
...
let make_deref loc (e, t) = 
  ...
  | TEany -> (Deref(loc, e), TEany)
  | _ -> report_expecting e "ref type" t

let make_uop loc uop (e, t) = 
  match uop, t with 
  | NEG, TEint | NEG, TEany  -> (UnaryOp(loc, uop, e), TEint) 
  | NEG, _      -> report_expecting e "integer" t
  | NOT, TEbool | NOT, TEany -> (UnaryOp(loc, uop, e), TEbool) 
  | NOT, _      -> report_expecting e "boolean" t

let ret_type bop = 
    match bop with
    | LT | OR | EQ  -> TEbool
    | ADD | SUB | MUL | DIV -> TEint
    | _ -> TEany
...
let make_bop loc bop (e1, t1) (e2, t2) = 
  ...
  | bop, TEany,  _        -> (Op(loc, e1, bop, e2), ret_type bop) 
  | bop, _,  TEany        -> (Op(loc, e1, bop, e2), ret_type bop) 
...
let make_while loc (e1, t1) (e2, t2)    = 
  if t1 = TEbool || t1 = TEany
  then if t2 = TEunit || t2 = TEany
...
let rec  infer env e = 
  ...
  | Try(loc, e1, x, e2) -> infer env e1
  | Raise(loc, e) -> infer env e (* TODO: Make exceptions have any type *)
